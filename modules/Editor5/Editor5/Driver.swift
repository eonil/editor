...
final class Driver {
    ...
    /// In AppKit, each `NSDocument`s are independent and disconnected
    /// island, and there's no way to deliver data between them.
    /// `NSApplication` and a few of other objects are same. 
    /// This method provides a channel to dispatch events from such 
    /// another islandish components.
