...
public extension Parsing {
	public class Rule {
		public typealias	Composition	=	(cursor:Cursor) -> Parsing.Stepping
		...
		func parse(cursor:Cursor) -> Parsing.Stepping {
...
public extension Parsing.Rule {
	...
	public enum Component {
		...
		public static func subrule(r1:Parsing.Rule)(cursor:Cursor) -> Parsing.Stepping {
		...
		public static func expect(composition c1:Composition, expectation e1:String?)(cursor:Cursor) -> Parsing.Stepping {
		...
		static func sequence(components:[Composition])(cursor:Cursor) -> Parsing.Stepping {
		...
		///	if `require` is `true`, repetition with insufficient occurrence will be treated as error
		///	instead of non-matching.
		static func choice(components:[Composition])(cursor:Cursor) -> Parsing.Stepping {
			var	c2	=	cursor
			var	ns1	=	NodeList()
			for comp in components {
				let	p1	=	comp(cursor: c2)
		...
		///	if `require` is `true`, repetition with insufficient occurrence will be treated as error
		///	instead of non-matching.
		static func repetition(unit:Composition, range:ClosedInterval<Int>)(cursor:Cursor) -> Parsing.Stepping {
			var	c2	=	cursor
			var	ns1	=	NodeList()
...
public func ~~~ (left:String, right:Parsing.Rule.Composition) -> Parsing.Rule {
	return	Parsing.Rule(name: left, composition: right)
}

public func + (left:Parsing.Rule.Composition, right:Parsing.Rule.Composition) -> Parsing.Rule.Composition {
	return	Parsing.Rule.Component.sequence([left, right])
}
public func | (left:Parsing.Rule.Composition, right:Parsing.Rule.Composition) -> Parsing.Rule.Composition {
	return	Parsing.Rule.Component.choice([left, right])
}
