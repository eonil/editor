...
public struct FileUtility {
	...
	///	An error on failure.
	public static func createNewFileInFolder(parentFolderURL:NSURL) -> Resolution<NSURL> {
		assert(parentFolderURL.existingAsDirectoryFile)
		
		let	u	=	parentFolderURL
//		let u2	=	parentFolder
//		let	u	=	u2.existingAsDirectoryFile ? u2 : u2.URLByDeletingLastPathComponent!
	...
	///	Returns URL to the created folder if succeeded.
	///	An error on failure.
	public static func createNewFolderInFolder(parentFolderURL:NSURL) -> Resolution<NSURL> {
		assert(parentFolderURL.existingAsDirectoryFile)
