//
//  LocalVariableTreeViewController.swift
...
final class LocalVariableTreeViewController: NSViewController, NSOutlineViewDataSource, NSOutlineViewDelegate {
...
//	}
	func outlineView(outlineView: NSOutlineView, viewForTableColumn tableColumn: NSTableColumn?, item: AnyObject) -> NSView? {
		...
		v.addConstraints([
			NSLayoutConstraint(item: tv, attribute: NSLayoutAttribute.Width, relatedBy: NSLayoutRelation.Equal, toItem: v, attribute: NSLayoutAttribute.Width, multiplier: 1, constant: 0),
			NSLayoutConstraint(item: tv, attribute: NSLayoutAttribute.Height, relatedBy: NSLayoutRelation.Equal, toItem: v, attribute: NSLayoutAttribute.Height, multiplier: 1, constant: 0),
			NSLayoutConstraint(item: tv, attribute: NSLayoutAttribute.CenterX, relatedBy: NSLayoutRelation.Equal, toItem: v, attribute: NSLayoutAttribute.CenterX, multiplier: 1, constant: 0),
			NSLayoutConstraint(item: tv, attribute: NSLayoutAttribute.CenterY, relatedBy: NSLayoutRelation.Equal, toItem: v, attribute: NSLayoutAttribute.CenterY, multiplier: 1, constant: 0),
			])
		
		////
		
		v.variableNodeValue	=	item as? VariableNode
		return	v
	}
...
private class ValueTableCellView: NSTableCellView {
	var variableNodeValue:VariableNode? {
		get {
			return	self.objectValue as VariableNode?
		}
		set(v) {
			self.objectValue	=	v
		}
	...
	override var objectValue:AnyObject? {
		...
		set(v) {
			precondition(v == nil || v! is VariableNode)
			super.objectValue	=	v
			
			if let n = v as? VariableNode {
				let	a	=	n.presentations[Column.Label]!
				let	b	=	n.presentations[Column.Kind]!
				let	c	=	n.presentations[Column.Expression]!
				
				let	sz	=	10 as CGFloat
				let	f	=	NSFont.systemFontOfSize(sz)
				let	f1	=	NSFont.boldSystemFontOfSize(sz)
//				let	f	=	NSFont(name: "Menlo", size: sz)!
//				let	f1	=	NSFont(name: "Menlo-Bold", size: sz)!
				...
				_originText			=	d.attributedString
				_highlightText		=	d.setTextColor(NSColor.selectedTextColor()).attributedString
			} else {
	...
	override var backgroundStyle:NSBackgroundStyle {
		didSet {
			...
			case .Dark:
				self.textField!.attributedStringValue	=	_highlightText
				
			case .Light:
				self.textField!.attributedStringValue	=	_originText
